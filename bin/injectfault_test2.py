...
################################################################################
def main(args):
  global optionlist, outputfile, totalcycles,run_id, return_codes, execute_num
  global defaultTimeout
  ...
  if not os.path.isfile(fi_exe):
    ...
    for ii, run in enumerate(rOpt):
      ...
      for index in range(0, run_number):
        ...
        if loopnum in execute_num:
          execute_num[loopnum] += 1
        else:
          execute_num[loopnum] = 1
        
        # subprocess.call('sh compileAndRun2.sh')
        
        os.system("sh ../sum_test4/compileAndRun3.sh")
        # os.system("sh compileAndRun3.sh")
        # os.system("sh compileAndRun2.sh")
      ...
      if options["verbose"]:
        ...
        print("Execute num: ")
        for r in list(execute_num.keys()):
          print(("  %3s: %5d" % (str(r), execute_num[r])))
